...
async def get_satellite_product(
    product_id: str = PathParam(..., description="Satellite product identifier (e.g., goes-19)")
):
    """
...
async def get_satellite_tile(
    ...
    # Validate zoom level
    is_valid, error_msg = satellite_service.validate_zoom_level(product_id, instrument_id, channel_id, z)
    if not is_valid:
        raise HTTPException(
    ...
    # Get tile path
    tile_path = satellite_service.get_tile_path(product_id, instrument_id, channel_id, tileset_id, z, x, y)
