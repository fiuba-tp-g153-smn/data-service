...
async def get_emas(
    ...
    time: Optional[list[str]] = Query(None, description="Time ranges (format: start-end:value)")
):
    """
    ...
    try:
        ...
        result = {
            slot: WeatherTimeSlot(features=[
                WeatherFeature(**feature) for feature in slot_data.get("features", [])
            ])
