...
def _snapshot_key(ts: datetime) -> str:
    ...
    return (
        f"{_S3_PREFIX}/snapshots/{ts.strftime('%Y/%m/%d/%H')}/{base}.json"
    )
...
class WeatherStationsScraperService(BaseSyncService):
    """
    Periodic 5-min scraper: fetches `/weather/station` (observations) +
    refreshes the public EMA registry, persisting both to S3.

    Lightweight by design: one provider, one endpoint, no Redis, no cursor,
    no circuit breaker. On HTTP failure for the observations endpoint we
    skip the cycle and try again next tick. The registry refresh is fully
    ...
    async def _run_sync(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "Weather stations scrape skipped: SMN fetch failed: %s", exc
            )
            return
        ...
        try:
            await self._upload_snapshot(scraped_at, snapshot_bytes, len(snapshot["stations"]))
        except Exception as exc:  # pylint: disable=broad-exception-caught
            logger.warning(
                "Weather stations scrape: S3 upload failed: %s", exc
            )
            return
    ...
    async def _maybe_bootstrap_registry_hash(self) -> None:
        ...
        if meta_bytes is not None:
            try:
                ...
                logger.warning(
                    "Stored registry meta unreadable, will rewrite: %s", exc
                )
    ...
    async def _refresh_registry_if_changed(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "Registry refresh skipped: %s", exc
            )
            return
        ...
        if not stations:
            logger.warning(
                "Registry refresh skipped: parser returned 0 stations"
            )
            return
