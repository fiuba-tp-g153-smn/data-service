...
class TestCatalogue:
    ...
    def test_mslp_is_abbreviated_in_both_the_url_and_s3(self):
        ...
        product = get_product("mean-sea-level-pressure")
    ...
    def test_only_500hpa_carries_barbs(self):
        assert GFS_PRODUCTS["geopotential-500hpa"].has_barbs
        assert not GFS_PRODUCTS["geopotential-250hpa"].has_barbs
        assert not GFS_PRODUCTS["mean-sea-level-pressure"].has_barbs
    ...
    def test_layers_never_advertise_barbs(self):
        ...
        assert layers_for("geopotential-500hpa") == ["heights", "isotherms"]

    def test_units_match_what_the_cogs_hold(self):
        assert GFS_PRODUCTS["mean-sea-level-pressure"].unit == "hPa"
        assert GFS_PRODUCTS["geopotential-500hpa"].unit == "kt"
        assert GFS_PRODUCTS["geopotential-250hpa"].unit == "kt"
...
class TestSecondaryVariables:
    ...
    def test_250_has_no_temperature(self):
        ...
        assert secondary_unit("geopotential-250hpa", "temperature") is None

    def test_units_mirror_the_overlays_they_twin(self):
        assert secondary_unit("mean-sea-level-pressure", "thickness") == "gpm"
        assert secondary_unit("geopotential-500hpa", "geopotential") == "gpm"
        assert secondary_unit("geopotential-500hpa", "temperature") == "°C"

    def test_unknown_variable_has_no_unit(self):
        """`None` is what stops an arbitrary segment reaching the key builder."""
        assert secondary_unit("geopotential-500hpa", "../../etc") is None
        assert secondary_unit("geopotential-500hpa", "vorticity") is None
...
class TestS3Keys:
    ...
    def test_tile_key(self):
        assert (
            S3Client.build_gfs_tile_key("geopotential-500hpa", CYCLE, FXXX, 5, 9, 17)
            == f"tiles/gfs/geopotential-500hpa/{CYCLE}/{STEP_ID}/5/9/17.webp"
        )

    def test_cog_key(self):
        assert (
            S3Client.build_gfs_cog_key("geopotential-250hpa", CYCLE, FXXX)
            == f"cog/gfs/geopotential-250hpa/{CYCLE}/{STEP_ID}.tif"
        )
    ...
    def test_mslp_keys_use_the_long_s3_segment(self):
        segment = GFS_PRODUCTS["mean-sea-level-pressure"].s3_segment
        key = S3Client.build_gfs_geojson_key(segment, CYCLE, FXXX, "isobars")
        assert key.startswith("geojson/gfs/mean-sea-level-pressure/")

    def test_step_id_joins_cycle_and_step(self):
        """The API splits them; tiles-processor names objects with them joined."""
        cog = S3Client.build_gfs_cog_key("geopotential-500hpa", CYCLE, FXXX)
    ...
    def test_secondary_cog_never_collides_with_the_primary(self):
        ...
        assert len(keys) == len(secondary_vars_for("geopotential-500hpa"))
    ...
    def test_secondary_cog_shares_the_cycle_prefix(self):
        ...
        key = S3Client.build_gfs_secondary_cog_key(
            "geopotential-250hpa", CYCLE, "geopotential", FXXX
        )
        assert key.startswith(S3Client.gfs_cog_cycle_prefix("geopotential-250hpa"))
...
class TestRedisTiles:
    ...
    async def test_products_do_not_collide(self):
        ...
        client = _client()
        await client.store_gfs_tile("geopotential-500hpa", CYCLE, FXXX, 5, 9, 17, b"a")
        await client.store_gfs_tile("geopotential-250hpa", CYCLE, FXXX, 5, 9, 17, b"b")
...
class TestRedisIndexes:
    ...
    async def test_add_index_writes_both_sorted_sets_with_ttl(self):
        ...
        await client.add_gfs_index("geopotential-500hpa", CYCLE, FXXX, 1.0, 3.0, 64800)
    ...
    async def test_steps_come_back_ascending(self):
        ...
        result = await client.get_gfs_steps("geopotential-500hpa", CYCLE)
        client._redis.zrange.assert_awaited_once_with(
            f"idx:gfs:geopotential-500hpa:{CYCLE}:steps", 0, -1
        )
    ...
    async def test_layers_are_sorted(self):
        ...
        assert await client.get_gfs_layers("geopotential-500hpa", CYCLE, FXXX) == [
            "heights",
            "isotherms",
        ]
    ...
    async def test_empty_layer_list_writes_nothing(self):
        client = _client()
        await client.add_gfs_layers("mean-sea-level-pressure", CYCLE, FXXX, [], 10)
