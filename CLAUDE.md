...
- **The broad availability check is one request, not one per product** (`/products/availability`, `services/product_availability_service.py`). Each domain registers an `AvailabilityContributor` keyed by the product's own API path, so a client looks up the same string it would have put in a probe URL. Radar is walked from its index in three pipelined round trips; satellite and GFS come from static catalogues; WRF is enumerated from `idx:wrf:products` (it has no static catalogue). A domain that answers nothing is left out of `domains` rather than reported as all-empty, so a cold index reads as *unknown* and the client re-probes instead of greying every row. The snapshot is memoised for `product_availability_ttl_seconds` so N clients collapse onto one walk. Individual per-product probes stay for the UI's single recheck buttons.
...
- Shared: `sync_mode`, `satellite_tile_ttl`, `radar_tile_ttl`, `radar_cache_control_tile_miss`, `tileset_listing_ttl`, `s3_max_concurrent_downloads`, `cache_control_config`, `cache_control_tile`, `product_availability_ttl_seconds`.
