...
| **Radar** | `/products/radar-sinarame/{radar_id}/{variable_id}/{elevation_id}/...` | `RadarService` | `tiles-data` bucket, prefix `tiles/radar/sinarame/` |
...
Derivation lives in `main.configure_basemap`. `relay_only` requires `basemap_online_fallback_enabled=true` (enforced by `Settings._validate`) and is the only mode that survives an S3 outage. Reader tier order is provider → Redis → S3 (`BasemapTileReader`): upstream is authoritative, the cached/backed-up copies answer when it fails. The provider tier is gated by a **per-provider `StorageCircuit`** (`basemap_relay_circuit_*`), because "authoritative" must not mean "re-probed on every tile": a host that blackholes TCP costs a full connect timeout per request, which without the breaker is re-paid per tile and pushes the chain past `basemap_request_deadline_seconds` — so the backup that would have answered in milliseconds is never reached and an upstream outage reads as a service outage. Once open, tier 1 is skipped until a half-open probe succeeds. Per-provider because providers fail independently; per-process because each Uvicorn worker holds its own reader. Operator-facing rationale per mode: README, "Basemap backup modes".
...
- Basemap: `basemap_backup_mode`, `basemap_providers`, `basemap_tile_ttl` (30 d default), `basemap_scrape_*` (incl. `basemap_scrape_parallelism_mode` — `sequential`/`per_origin`/`full` — and `basemap_scrape_per_host_concurrent`, a per-host request budget stacked under `basemap_scrape_concurrent`), `basemap_provider_cooldown_schedule` + `basemap_provider_error_rate_*` (circuit breaker — trips on per-sweep UNAVAILABLE error rate, exponential-backoff cooldown, state persisted in SQLite `basemap_provider_health`), `basemap_cache_*`, `basemap_bbox_*`, `basemap_http_*`, `basemap_reader_http_*`, `basemap_request_deadline_seconds`, `basemap_relay_circuit_*` (reader-side tier-1 breaker — opens after `min_samples` outcomes exceed `threshold` failures, backs off to `max_cooldown`; distinct from the scraper's `basemap_provider_*` breaker), `basemap_s3_object_ttl_days` (35 d default — strictly greater than scrape interval), `basemap_online_fallback_enabled`, `basemap_provider_availability_ttl`, `basemap_scrape_state_db_path`, `basemap_cache_control_tile_miss`.
